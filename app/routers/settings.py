...
def test_hermes(payload: TestHermesRequest, db: Session = Depends(get_db)):
    ...
    if mode == "local":
        base_url = (payload.hermes_api_url or settings.get("hermes_local_api_url") or "http://127.0.0.1:8642").strip().rstrip("/")
        api_key = payload.hermes_api_key or settings.get("hermes_local_api_key") or ""
    else:
...
# ---------------------------------------------------------------------------
# 本机 Hermes 网关控制（multiplex 单共享网关）
# ---------------------------------------------------------------------------
@router.get("/local-gateway/status")
def local_gateway_status():
    from ..local_hermes import LocalHermesGateway
    return LocalHermesGateway.instance().status()


@router.post("/local-gateway/start")
def local_gateway_start(db: Session = Depends(get_db)):
    import secrets

    from ..local_hermes import LocalHermesGateway
    from ..services import _resolve_hermes_profiles_dir
    from ..settings_store import update_settings
    settings = get_settings(db)

    # api_server 要求 API_SERVER_KEY 必填；用户没填时自动生成并回写到设置，
    # 保证执行端（HermesExecutor）用同一个 key 调用。
    api_key = (settings.get("hermes_local_api_key") or "").strip()
    autogenerated = not api_key
    if autogenerated:
        api_key = secrets.token_urlsafe(24)
        update_settings(db, {"hermes_local_api_key": api_key})

    result = LocalHermesGateway.instance().start(
        hermes_bin=settings.get("hermes_bin") or "hermes",
        api_url=settings.get("hermes_local_api_url") or "http://127.0.0.1:8642",
        profiles_root=str(_resolve_hermes_profiles_dir(settings)),
        api_key=api_key,
    )
    if autogenerated:
        result["api_key_autogenerated"] = True
    return result


@router.post("/local-gateway/stop")
def local_gateway_stop():
    from ..local_hermes import LocalHermesGateway
    return LocalHermesGateway.instance().stop()
